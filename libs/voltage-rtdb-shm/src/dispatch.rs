...
//! Domain Socket notifications. Lives in voltage-rtdb-shm so both modsrv
//! (HTTP /control path) and voltage-rules (rule engine actions) can share
...
//! and silently write to stale slots after a comsrv restart.
...
impl DispatchOutcome {
    /// Returns true if the dispatch reached comsrv (Delivered or Noop).
    /// Callers use this to gate Redis mirror writes and cooldown counting.
    #[inline]
...
impl ActionDispatch for ShmDispatch {
    async fn dispatch(&self, ctx: &RouteContext, value: f64) -> DispatchOutcome {
        ...
        // Generation pre-check: detect comsrv restarts that changed the SHM layout.
