<template>
  <div class="pv__content">
    <div v-if="dgChIds.length > 1" class="channel-toolbar">
    <span class="channel-toolbar__label">Channel:</span>
      <el-select
        v-model="selectedDgChId"
        size="small"
        class="channel-toolbar__select"
        fit-input-width
      >
        <el-option v-for="id in dgChIds" :key="id" :label="String(id)" :value="id" />
      </el-select>
    </div>
...
const dgChIds = computed(() => {
  const selectedId = topoStore.selectedDieselId
  return topoStore.bindings
    .filter((binding) => binding.productName === 'Diesel')
    .flatMap((binding) => binding.instances)
    .filter((instance) => selectedId === null || instance.instanceId === selectedId)
    .flatMap((instance) => instance.channelIds)
    .filter((id, index, ids) => ids.indexOf(id) === index)
})
const selectedDgChId = ref<number>()
const dgChId = computed<number | undefined>(() => selectedDgChId.value)
...
watch(
  dgChIds,
  (ids) => {
    if (!ids.includes(selectedDgChId.value ?? -1)) selectedDgChId.value = ids[0]
  },
  { immediate: true },
)
watch(
